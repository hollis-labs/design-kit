...
interface TransferListProps {
   ...
   *
   * sysop-ui hardcoded `"Search models"` here — a domain leak CW-0116 flagged:
   * the component is a generic dual-pane picker and nothing about it knows what a
   * model is. Copy that names the caller's domain is the same class of defect as
   * a token that names a colour, one layer up.
   */
...
function TransferListPane({
  ...
  return (
    <div className="flex min-h-0 flex-col rounded-lg border border-border bg-bg/40">
      <div className="border-b border-border px-3 py-2">
        <div className="flex items-center justify-between gap-2">
          <div className="text-label uppercase tracking-label text-fg-faint">{title}</div>
                ...
                <button
                  ...
                  className={cn(
                    ...
                    selected
                      ...
                      : 'border-border bg-bg hover:border-border hover:bg-bg/70',
                  )}
                >
                  <div className="font-mono text-xs text-fg">{item.label}</div>
                  {item.description ? (
                    <div className="mt-1 text-label text-fg-muted">{item.description}</div>
                  ) : null}
                  {item.meta ? (
                    <div className="mt-1 text-caption uppercase tracking-label text-fg-faint">
                      {item.meta}
                    </div>
                  ) : null}
                </button>
